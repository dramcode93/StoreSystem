...
function UpdateProduct() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: productData } = await axios.get(`https://rich-blue-ladybug-robe.cyclic.app/api/products/${id}`, {
          headers: {
  ...
  const handleUpdateProduct = () => {
    axios.put(`https://rich-blue-ladybug-robe.cyclic.app/api/products/${id}`, { name: newProductName, price: newProductPrice, quantity: newProductQuantity, category: selectedCategoryId }, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
