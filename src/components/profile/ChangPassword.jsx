...
const ChangePassword = () => {
  ...
  return (
    <div className={`bg-gray-700 bg-opacity-25 mx-10 rounded-md pt-2 absolute top-32 -z-3 w-3/4 p-4 ${language === "ar" ? "left-10" : "right-10"}`} >
      <h3 className='font-bold text-white'>{t(`ChangePasswordPage.ChangePasswordPage`)}</h3>
      <form className=''>
        <label htmlFor='password'>{t(`ChangePasswordPage.CurrentPassword`)}</label>
        ...
        <button onClick={handleEditPassword} className="bg-yellow-900  rounded-lg hover:bg-yellow-800 fw-bold">{t(`ChangePasswordPage.ChangePassword`)}</button>
      </form>
